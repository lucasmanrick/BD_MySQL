...
SELECT tu.nome as nomeComprador, tp.nome as produto, tc.tipo as categoria, tp.descricao, tp.console, tp.preco FROM tb_produtos as tp JOIN tb_usuarios as tu ON tp.usuario_id = tu.id JOIN tb_categorias AS tc ON tp.categoria_id = tc.id WHERE preco >= 500 order by nomeComprador;